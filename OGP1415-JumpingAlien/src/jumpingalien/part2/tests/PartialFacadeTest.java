...
public class PartialFacadeTest {
	...
	public void testAddShark() {
		...
		World world = facade.createWorld(50, 3, 3, 1, 1, 1, 1);
		Shark shark = new Shark(1,1, spriteArrayForSize(3, 3));
		world.addSharkToCollection(shark);
		assertTrue(world.getCollectionOfSharks().contains(shark));
	}
	
	@Test
	public void testAddSharkClone() {
		IFacadePart2 facade = new Facade();

		World world = facade.createWorld(50, 3, 3, 1, 1, 1, 1);
		Shark shark = new Shark(1,1, spriteArrayForSize(3, 3));
		world.addSharkToCollection(shark);
		world.getCollectionOfSharks().remove(shark);
		assertTrue(world.getCollectionOfSharks().contains(shark));
	}
	
	@Test
	public void testAddSharkClone_LegalCase() {
		IFacadePart2 facade = new Facade();

		World world = facade.createWorld(50, 3, 3, 1, 1, 1, 1);
		Shark shark = new Shark(1,1, spriteArrayForSize(3, 3));
		world.addSharkToCollection(shark);
		world.setInvisibleWorldObject(shark);
		assertTrue(!world.getCollectionOfSharks().contains(shark));
	}
	
	
	@Test
	public void testTilesWidthInPixels() {
		IFacadePart2 facade = new Facade();

		World world = facade.createWorld(50, 3, 3, 1, 1, 1, 1);
		// 50*3 = 150
		assertEquals(world.getWidthInPixel(), 150);
	}
	
	@Test
	public void testTilesHeightInPixels() {
		IFacadePart2 facade = new Facade();

		World world = facade.createWorld(50, 3, 3, 1, 1, 1, 1);
		// 50*3 = 150
		assertEquals(world.getHeightInPixel(), 150);
	}
	
	@Test
	public void testTilesBottomLeftPixel() {
		IFacadePart2 facade = new Facade();
		...
		array[0] = 50;
		array[1] = 50;
		assertArrayEquals(world.getBottomleftPixelOfTile(1, 1), array);
	}
	...
	public void startMoveLeftMaxSpeedAtRightTime() {
		...
		World world = facade.createWorld(500, 1, 2, 1, 1, 1, 1);
		facade.setGeologicalFeature(world, 0, 0, FEATURE_SOLID);
		Mazub alien = facade.createMazub(500, 499, spriteArrayForSize(3, 3));
		facade.setMazub(world, alien);
		facade.startMoveLeft(alien);
		// maximum speed reached after 20/9 seconds
		for (int i = 0; i < 100; i++) {
